...
### Agent 2.0：从“生成答案”升级为“完成规划任务”

本版本不再只把用户消息转发给模型，而是在模型前后加入可观察、可测试的 Agent 工作流：

- **旅行条件记忆**：把日期、到达时间、可用时长、同行人、偏好、节奏与排除项保存为结构化会话状态；
- **确定性工具调用**：依次执行条件提取、本地知识检索、路线约束检查和版本差异比对；
- **规划—校验—修复循环**：模型先生成候选路线，服务端检查时间顺序、重复站点、时长和排除项；失败时自动定向修复一次；
- **局部路线修改**：用户可直接说“晚到 30 分钟”“节奏更轻松”或“避开刺激项目”，系统保留未受影响的站点，只调整相关部分；
- **可见的 Agent 证据**：页面展示已记住的条件、真实执行阶段、知识引用数量、约束检查结果与路线改动摘要。
...
flowchart LR
    ...
    K --> P["规划 / 局部改线"]
    P --> V{"约束检查"}
    V -- "通过" --> C["路线卡片 + Agent 证据"]
    V -- "冲突" --> R["定向修复 1 次"]
    R --> V
~~~

会话状态由浏览器随每次请求携带，后端保持无状态，因此无需数据库即可部署到 Render；状态中不包含 API Key 或个人敏感信息。
...
flowchart LR
    U["用户"] --> W["网页对话界面"]
    W --> API["FastAPI Agent 编排服务"]
    API --> S["会话条件 / 工具编排"]\n    S --> RAG["澳门塔知识库 / BM25 检索"]
...
- 澳门塔本地知识库与 BM25 检索
- Pydantic 结构校验与确定性路线约束检查\n- 规划—校验—修复循环与局部路线调整
- 结构化路线 JSON 与普通文本回复兼容

### 部署

- GitHub Pages：公开网站
- Render：FastAPI Agent 服务部署与验证
...
- 路线结构可被校验并渲染为卡片；
- 模型异常时可以进入降级流程；
- 前端 GitHub Pages 与后端 Render 部署流程通过。\n- Agent 条件提取、约束冲突检测与路线差异比对具备自动化测试。
